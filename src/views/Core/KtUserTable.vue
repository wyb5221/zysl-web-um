<template>
  <div>
    ...
    <el-table :data="data.content" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange"
      ...
      <el-table-column :label="$t('action.operation')" width="285" fixed="right" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope">
          ...
          <kt-button icon="icon iconfont" :label="$t('action.unlock')" :perms="permsUnlock" :disabled="scope.row.status!=2" :size="size" type="danger" @click="handleUnlock(scope.$index, scope.row)"/>
        </template>
      </el-table-column>
    </el-table>
    <!--分页栏-->
      ...
      this.$confirm('确认解锁', '提示', {
        ...
        let callback = res => {
          if(res.code == 200) {
            ...
            this.$message({message: '解锁失败, ' + res.msg, type: 'error'})
          }
          this.loading = false
        }
        this.$emit('handleUnlock', {params:params, callback:callback})
      }).catch(() => {
      })
    },
		// 删除操作
